...
namespace ASE
{
    ...
    void resampleCloud(typename PointCloud<PointT>::Ptr& cloud);

    PointCloud<PointXYZ>::Ptr applyNoiseFilter(const PointCloud<PointXYZ>::ConstPtr& cloud, int kNearestNeighbor = 100, double stdDevMul = 0.5);    
    ...
    PointCloud<PointXYZ>::Ptr subtract(const PointCloud<PointXYZ>::ConstPtr& left, const PointCloud<PointXYZ>::ConstPtr& right, float precision = 0.1);
    ...
    BoundingBox computeOBB(const typename PointCloud<PointIn>::ConstPtr& cloud);
    
    template<typename PointIn>
    BoundingBox computeAABB(const typename PointCloud<PointIn>::ConstPtr& cloud);
    ...
    void resampleCloud(typename PointCloud<PointT>::Ptr& cloud)
    {
        auto bb = computeOBB<PointT>(cloud);

        // compute maximum leaf size of the grid that causes no integer overflow
        double ls = (bb.sizeX * bb.sizeY * bb.sizeZ) / numeric_limits<unsigned int>::max() - numeric_limits<double>::epsilon();
    ...
    inline PointCloud<PointXYZ>::Ptr applyNoiseFilter(const PointCloud<PointXYZ>::ConstPtr& cloud, int kNearestNeighbor, double stdDevMul)
    {
    ...
    inline PointCloud<PointXYZ>::Ptr applyClusterFilter(const PointCloud<PointXYZ>::ConstPtr& cloud, double clusterTolerance, int minClusterSize, int maxClusterSize)
    {
        PointCloud<PointXYZ>::Ptr result(new PointCloud<PointXYZ>);
    ...
    inline PointCloud<PointXYZ>::Ptr subtract(const PointCloud<PointXYZ>::ConstPtr& left, const PointCloud<PointXYZ>::ConstPtr& right, float precision)
    {
        PointCloud<PointXYZ>::Ptr result(new PointCloud<PointXYZ>);
        ...
        int iterationsPerUpdate, function<bool(int, double, Matrix4f&)> updateCallback)
    {
        ...
        for (int iterCnt = 1; iterCnt <= maxIterations; iterCnt += iterationsPerUpdate)
        {
            ...
            if (fitnessEpsilon > 0 && fitness - f < fitnessEpsilon)
                break;
    ...
    inline void runIcp(PointCloud<PointXYZ>::Ptr source, PointCloud<PointXYZ>::Ptr target, double fitnessEpsilon, int maxIterations,
        int iterationsPerUpdate, function<bool(int, double, Matrix4f&)> updateCallback) 
    {
    ...
    BoundingBox computeOBB(const typename PointCloud<PointIn>::ConstPtr& cloud)
    {
        ...
        moments.compute();
        volatile bool xxx = moments.getOBB(a, g, pos, rot);
        s = size(g, a);
        b = { a.x + s.x, a.y, a.z };
