...
pcl::PointCloud<pcl::PointXYZ>::Ptr MeshSampling::sample(pcl::PolygonMesh& mesh, int pointCount, float leafSize)
{
...
void MeshSampling::_randomPointTriangle(float a1, float a2, float a3, float b1, float b2, float b3, float c1, float c2, float c3, Eigen::Vector4f& p)
{
...
void MeshSampling::_randPSurface(vtkPolyData* polydata, std::vector<double>* cumulativeAreas, double totalArea, Eigen::Vector4f& p)
{
...
void MeshSampling::_uniformSampling(vtkSmartPointer<vtkPolyData> polydata, size_t n_samples, pcl::PointCloud<pcl::PointXYZ>& cloud_out)
{
    polydata->BuildCells();
